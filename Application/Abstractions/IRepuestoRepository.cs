...
public interface IRepuestoRepository
{
    ...
    Task<IReadOnlyList<Repuesto>> GetPagedAsync(int pageNumber, int pageSize, string? search = null, int? categoriaId = null, CancellationToken ct = default);
    Task<int> CountAsync(string? search = null, int? categoriaId = null, CancellationToken ct = default);
